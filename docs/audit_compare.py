...
api_data = {
    ...
    '2026-08-27': {'lines': 0, 'qty': 0, 'nett_sales': 0, 'bills': 0},
...
print(f"{'Date':<12} {'ExcelRows':>10} {'APIRows':>8} {'Diff':>7} {'%Diff':>7} {'Status':<12}")
print('-' * 70)
...
for fname in files:
    ...
    if api_lines == 0 and excel_rows > 0:
        status = 'MISSING'
        missing_dates.append((date, excel_rows))
    elif diff > 0:
        status = f'EXCESS +{diff}'
        excess_dates.append((date, diff))
    else:
        status = f'OK ({pct:.1f}%)'

    print(f"{date:<12} {excel_rows:>10,} {api_lines:>8,} {diff:>+7,} {pct:>+6.1f}% {status}")
...
print(f"MISSING dates ({len(missing_dates)}): {[(d,r) for d,r in missing_dates]}")
print(f"EXCESS dates ({len(excess_dates)}): {[(d,r) for d,r in excess_dates]}")
